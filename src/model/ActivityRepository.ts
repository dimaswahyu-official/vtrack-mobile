...
export const dropTableExisting = async (
...
export const ActivityRepository = {
	create: async (
		...
		const result = await db.runAsync(
			`INSERT INTO Activity (user_id, call_plan_id, call_plan_schedule_id, outlet_id, survey_outlet_id, program_id, status, area, region, brand, type_sio, latitude, longitude, start_time, end_time, photo, photo_program, sale_outlet_weekly, is_sync)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[
				user_id ?? 0,
				call_plan_id ?? 0,
