...
export const createTableActivityProgram = async (
...
export const ActivityProgramModel = {
	...
	create: async (
		db: SQLite.SQLiteDatabase,
		params: ActivityProgramCreateParams
	): Promise<number> => {
		const { call_plan_schedule_id, name, description, photo, is_sync } =
			params;
		...
		try {
			const result = await db.runAsync(
				`INSERT INTO ActivityProgram (call_plan_schedule_id, name, description, photo, is_sync)
                 VALUES (?, ?, ?, ?, ?)`,
				[call_plan_schedule_id, name, description, photo, is_sync ?? 0]
			);
	...
	update: async (
		...
		await db.runAsync(`UPDATE ActivityProgram SET ${fields} WHERE id = ?`, [
			...values,
			params.id,
		]);
	},
